...
public class DeleteGroupSubCommandTest {
    ...
    public void shouldProperlyDeleteByGroupId() {
        ...
        gs1.setUsers(users);
        Mockito.when(groupSubService.findById(String.valueOf(groupId))).thenReturn(Optional.of(gs1));
        Mockito.when(telegramUserService.findByChatId(chatId))
                .thenReturn(Optional.of(telegramUser));
    ...
    public void shouldDoesNotExistByGroupId() {
        ...
        Mockito.when(groupSubService.findById(String.valueOf(groupId))).thenReturn(Optional.empty());

        String expectedMessage = "Не нашел такой группы =/";

        command.execute(update);

        Mockito.verify(groupSubService).findById(String.valueOf(groupId));
        Mockito.verify(sendBotMessageService).sendMessage(chatId, expectedMessage);
    }
}
